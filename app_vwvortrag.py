...
## Epidemic Model
'''
option_model = st.selectbox('Spreading Model', ('Corona model', 'SI model', 'SEIRS model', 'Custom'))
...
option_network = st.selectbox('Contact Network', ( 'Geometric', 'Karate', '2D-Grid', 'Custom'))
pos = None
...
if option_network == 'Karate':
    ...
    nodenum = st.slider('Node-Num', min_value=1.0, max_value=400.0, value=250.0, step=1.0, key='nodenum')
...
for s in states_eval:
    ...
    if s == "I" or s == "I1":
        ...
        value = G.number_of_nodes() -2.0 - 20
    elif s == 'V':
        value = 20.0
...
## Run Simulation
'''
horizon_slider = st.slider('Horizon', min_value=1.0, max_value=200.0, value=50.0, key='H')
        ...
        with imageio.get_writer('movie.gif', mode='I', fps=2.0) as writer:
            for filename in sorted(glob.glob('imagedata/*.png')):
                try:
                    ...
                    print('Image load errotr')

        # st.markdown("![Alt Text](./movie.gif)")
        st.image("movie_{}.gif".format(int(random.random()*10000)))
